...
def batch_similarity_ground_truth(valiortest = None):
    ...
    if valiortest == 'vali':
        node_list_int = all_node_list_int[config["train_set_size"]:config["vali_set_size"]]
    elif valiortest == 'test':
        node_list_int = all_node_list_int[config["vali_set_size"]:config["test_set_size"]]
...
def merge_similarity_ground_truth(sample_len, valiortest):
    ...
    res = np.concatenate(res, axis=0)
    np.save('./ground_truth/{}/{}/{}_spatial_distance_{}.npy'.format(dataset, str(config["distance_type"]), valiortest, str(config["dataset_size"])), res)
...
if __name__ == '__main__':
    nx_vertice, _, vertice_dict, _, edge_dist, edge_dist_dict, roadnetwork = network_data()
